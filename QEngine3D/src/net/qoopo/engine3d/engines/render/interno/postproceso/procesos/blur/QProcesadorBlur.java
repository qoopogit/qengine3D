...
//    private static int mitad = 15;
    ...
    public void procesar(QTextura... buffer) {
        ...
        for (int i = 1; i <= repeticiones; i++) {
            bufferSalida = transposedHBlur(transposedHBlur(bufferSalida, ancho, alto), alto, ancho);
        }
    }

    //https://stackoverflow.com/questions/43743998/how-to-make-smooth-blur-effect-in-java
    public static QTextura transposedHBlur(QTextura textura, int ancho, int alto) {
        ...
        for (int y = 0; y < textura.getAlto(); y++) {
            for (int x = MITAD; x < textura.getAncho() - MITAD; x++) {
                pixel.set(1, 0, 0, 0);
                for (int i = 0; i < KERNEL_SIZE; i++) {
                    pixel = pixel.add(textura.getColor(x + i - MITAD, y).scale(PESOS[i]));
                }
                // transpose result!
