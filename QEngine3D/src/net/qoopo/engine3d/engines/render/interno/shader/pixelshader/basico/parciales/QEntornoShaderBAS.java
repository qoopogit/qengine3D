...
//    private QColor colorDesplazamiento;
    private float alpha;
    private float factorReflexion = 0;
...
//        }
        ...
        if (((QMaterialBas) ((QMaterialBas) pixel.material)).isTransparencia()) {
            //si tiene un mapa de transparencia
            if (((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaTransparencia() != null) {
                // es una imagen en blanco y negro, toma cualquier canal de color
                alpha = ((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaTransparencia().get_QARGB(pixel.u, pixel.v).r;
            } else {
                //toma el valor de transparencia del material
                alpha = ((QMaterialBas) ((QMaterialBas) pixel.material)).getTransAlfa();
            }
        } else {
            alpha = 1;
        }

        if (((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaDifusa() == null || !render.opciones.material) {
            // si no hay textura usa el color del material
            color.set(((QMaterialBas) ((QMaterialBas) pixel.material)).getColorDifusa());
        } else {

            if (!((QMaterialBas) ((QMaterialBas) pixel.material)).isDifusaProyectada()) {
                //si la textura no es proyectada (lo hace otro renderer) toma las coordenadas ya calculadas 
                colorDifuso = ((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaDifusa().get_QARGB(pixel.u, pixel.v);
            } else {
                ...
                // por lo tanto convertimos las coordeandas XyY a coordenads UV 
                colorDifuso = ((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaDifusa().get_QARGB((float) x / (float) render.getFrameBuffer().getAncho(), -(float) y / (float) render.getFrameBuffer().getAlto());
            }

            switch (((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaDifusa().getModo()) {
                case QProcesadorTextura.MODO_COMBINAR:
                    color.r = (colorDifuso.r + ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorDifusa().r) / 2;
                    color.g = (colorDifuso.g + ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorDifusa().g) / 2;
                    color.b = (colorDifuso.b + ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorDifusa().b) / 2;
                    break;
            ...
            pixelTransparente2 = ((QMaterialBas) ((QMaterialBas) pixel.material)).isTransparencia()
                    && ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorTransparente() != null
                    && colorDifuso.toRGB() == ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorTransparente().toRGB();//sin alfa
...
//                si es totalmente transparente no se dibuja
        ...
        if (((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaEspecular() != null) {
            colorEspecular = ((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaEspecular().get_QARGB(pixel.u, pixel.v);
            factorReflexion = colorEspecular.r;
        } else {
            colorEspecular = QColor.WHITE;//equivale a multiplicar por 1
            factorReflexion = ((QMaterialBas) ((QMaterialBas) pixel.material)).getFactorEntorno();
        }
...
//        }
        ...
        //******                    TRANSPARENCIA
        //***********************************************************
        if (((QMaterialBas) ((QMaterialBas) pixel.material)).isTransparencia() && alpha < 1) {
            // si el pixel tiene transparencia (canal alfa de imagenes PNG)
            QColor tmp = render.getFrameBuffer().getColor(x, y);
...
//        render.getFrameBuffer().setQColor(x, y, color);
    ...
    /**
     * Calcula la Reflexión y Refracción del mapa de entorno
     *
     * @param currentPixel
     */
...
//                tm.vector3f1.set(QTransformar.transformarVectorModelo(tm.vector3f1, currentPixel.entidad));
...
//                tm.vector3f1.add(render.getCamara().getMatrizTransformacionTMP().toTranslationVector().multiply(-1.0f));
                ...
                if (((QMaterialBas) currentPixel.material).isReflexion()) {
                    ...
                    tm.vector3f3.set(QTransformar.transformarVectorNormalInversa(tm.vector3f3, currentPixel.entidad, render.getCamara()));
...
//                    tm.vector3f4.set(QMath.refractarVector(tm.vector3f1, tm.vector3f2, 1.0f / ((QMaterialBas) currentPixel.material).getIndiceRefraccion())); //indice del aire sobre indice del material
                    ...
                    tm.vector3f4.set(QTransformar.transformarVectorNormalInversa(tm.vector3f4, currentPixel.entidad, render.getCamara()));
                ...
                if (colorReflejo != null && colorRefraccion != null) {
                    if (QGlobal.REFLEJOS_CALCULAR_FRESNEL) {
                        factorFresnel = QMath.factorFresnel(tm.vector3f1, tm.vector3f2, 0);// 1.0f / ((QMaterialBas) currentPixel.material).getIndiceRefraccion());
                    } else {
...
//                b += ((QMaterialBas) currentPixel.material).getFactorReflexion() * colorEspecular.b*colorEntorno.b;
                ...
                color.r = QMath.mix(color.r, colorEntorno.r, factorReflexion);
                color.g = QMath.mix(color.g, colorEntorno.g, factorReflexion);
                color.b = QMath.mix(color.b, colorEntorno.b, factorReflexion);
                //             
//                
//                color.r = QMath.mix(color.r, colorEspecular.r*colorEntorno.r, ((QMaterialBas) currentPixel.material).getFactorReflexion());
//                color.g = QMath.mix(color.g, colorEspecular.g*colorEntorno.g, ((QMaterialBas) currentPixel.material).getFactorReflexion());
//                color.b = QMath.mix(color.b, colorEspecular.b*colorEntorno.b, ((QMaterialBas) currentPixel.material).getFactorReflexion());
...
//    @Override
    protected void calcularIluminacion(QIluminacion illumination, QPixel pixel) {
        ...
        if (((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaEmisivo() != null && render.opciones.material) {
            QColor colorEmisivo = ((QMaterialBas) ((QMaterialBas) pixel.material)).getMapaEmisivo().get_QARGB(pixel.u, pixel.v);
            ...
            illumination.dR = (((QMaterialBas) ((QMaterialBas) pixel.material)).getFactorEmision() + render.getEscena().getLuzAmbiente());
            illumination.dG = (((QMaterialBas) ((QMaterialBas) pixel.material)).getFactorEmision() + render.getEscena().getLuzAmbiente());
            illumination.dB = (((QMaterialBas) ((QMaterialBas) pixel.material)).getFactorEmision() + render.getEscena().getLuzAmbiente());
        }
                ...
                for (QLuz luz : render.getLuces()) {
                    ...
                    if (luz != null && luz.entidad.isRenderizar() && luz.isEnable()) {
                        if (luz instanceof QLuzPuntual || luz instanceof QLuzSpot) {
                            ...
                            if (proc != null && render.opciones.sombras && ((QMaterialBas) ((QMaterialBas) pixel.material)).isSombrasRecibir()) {
...
//                        if (!verificarSombra(vectorLuz, light.entidad.transformacion.getTraslacion().x, light.entidad.transformacion.getTraslacion().y, light.entidad.transformacion.getTraslacion().z, x, y, z)) {
                            ...
                            if (vectorLuz.dotProduct(pixel.normal) < 0) {
                                ...
                                if (((QMaterialBas) ((QMaterialBas) pixel.material)).getSpecularExponent() > 0) {
                                    if (iluminacionEspecular > 0) {
                                        iluminacionEspecular = (float) Math.pow(iluminacionEspecular, ((QMaterialBas) ((QMaterialBas) pixel.material)).getSpecularExponent());
                                        iluminacionEspecular = iluminacionEspecular * luz.energia / distanciaLuz;
                                        illumination.sR += colorEspecular.r * ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorEspecular().r * iluminacionEspecular * factorSombra;
                                        illumination.sG += colorEspecular.g * ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorEspecular().g * iluminacionEspecular * factorSombra;
                                        illumination.sB += colorEspecular.b * ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorEspecular().b * iluminacionEspecular * factorSombra;
                                    }
                                }
                            }
                            ...
                            if (vectorLuz.dotProduct(pixel.normal) < 0) {
                                ...
                                if (iluminacionEspecular > 0) {
                                    iluminacionEspecular = (float) Math.pow(iluminacionEspecular, ((QMaterialBas) ((QMaterialBas) pixel.material)).getSpecularExponent());
                                    illumination.sR += colorEspecular.r * luz.energia * ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorEspecular().r * iluminacionEspecular * factorSombra;
                                    illumination.sG += colorEspecular.g * luz.energia * ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorEspecular().g * iluminacionEspecular * factorSombra;
                                    illumination.sB += colorEspecular.b * luz.energia * ((QMaterialBas) ((QMaterialBas) pixel.material)).getColorEspecular().b * iluminacionEspecular * factorSombra;
                                }
                            }
                        }
                    }
